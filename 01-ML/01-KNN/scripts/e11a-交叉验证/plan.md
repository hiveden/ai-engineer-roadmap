...
- `05-hyperparameter/demos/01-cv-gridsearch.py` Tab 1（CV 折数 vs 单次拆分方差对比）

**实际 UI**（按这个写录屏稿，**已读 demo 代码**）：
- **3 个 Tab**：① CV 折数对方差 ② GridSearchCV 热力图 ③ 手写数字 KNN（**本期只用 Tab 1**，Tab 2/3 留给 e11b）
- **Tab 1 控件**（hstack）：
...
| 6 | content | **Demo · CV 折数对方差**（Tab 1 实测对比） | **B** | ~340 |
