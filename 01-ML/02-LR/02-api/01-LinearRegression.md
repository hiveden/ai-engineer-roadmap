...
> 级别：【实操】 · 维度：代码 · 底稿见 [`README.md`](./README.md)（slide 15-17）

## 底稿（精简版 PPT 原文 + 完整代码）

需求：5 个身高-体重样本，预测身高 176 的体重。

5 步流程：

1. **导入** `from sklearn.linear_model import LinearRegression`
2. **准备数据** x 必须 2D（n 样本 × n 特征），y 1D
3. **实例化** `estimator = LinearRegression()`
4. **训练** `estimator.fit(x, y)`
5. **预测** `estimator.predict([[176]])`

查看模型：`coef_`（斜率/权重 w）、`intercept_`（截距 b）。
...
**`LinearRegression()` 就是把 [`01-intro/02-线性回归`](../01-intro/02-线性回归.md) 的公式 $\hat{y} = \mathbf{w}^T \mathbf{x} + b$ 装进一个对象** —— `fit()` 算 w 和 b，`predict()` 套公式。整套 API 4 行能跑通。
...
解析解对量纲不敏感（数学上 $w_i$ 会自动缩放抵消），但**多元 + 量纲悬殊**时数值精度会受影响。建议养成 `StandardScaler` + LR 的习惯，详见 [`04b-scaling/`](../04b-scaling/)。
...
- 特征缩放配合 → [`04b-scaling/`](../04b-scaling/)
