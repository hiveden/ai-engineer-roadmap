...
### 生活锚 · 装修一套房子，10 个工种串起来

老房子翻新，业主手里只有一份户型图，但施工队进场后是一条接力流水线：

1. **拆改师傅**：砸非承重墙、铲旧瓷砖（对应：扔掉 ID 列、清掉 11 行带空格的脏数据）
2. **水电工**：埋管布线，画施工图（对应：把字符串列翻译成 0/1 多列）
3. **泥瓦工**：地面找平、贴砖（对应：把月费 0–119、总费用 0–8000 这种悬殊数字拉到同一量纲）
4. **木工 + 油漆工**：打柜子、刷墙（对应：训练模型 + 调参）
5. **总监理**：每段验收签字、最后整屋验房（对应：交叉验证 + AUC 评估）
6. **业主入住前**：再请第三方甲醛检测（对应：测试集上独立看一次 Recall）

每个工种有自己专属的工具、顺序和验收标准，串错一步——比如先刷墙后埋水电——后面整面墙都得敲了重来。本节就是把这 10 个工种对到 sklearn 一份 5 步 pipeline 上，看每一行代码背后的"工种取舍"。

### 业务问题

数据团队照着 02 节流水线写了三段代码（数据处理 / 特征筛选 / 模型训练），跑出 AUC 0.78 左右的初版结果。运营经理签字之前会追问：为什么这一行用 coerce 不用 astype？为什么测试集要按流失比例切？为什么调参看的是 AUC 不是准确率？

本节不重抄代码，而是讲**"为什么这么写"**——每一行背后的工程取舍。
