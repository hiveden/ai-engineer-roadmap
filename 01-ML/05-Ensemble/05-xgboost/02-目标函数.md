...
| $g_i$ | g-i | 损失对 $\hat{y}_i^{(t-1)}$ 的一阶导 | 残差方向（斜率） |
| $h_i$ | h-i | 损失对 $\hat{y}_i^{(t-1)}$ 的二阶导 | 曲率（弯曲程度） |
| $T$ | 大写 T | 当前树的叶子总数 | 二叉树深度 3 → 最多 8 叶 |
| $w_j$ | w-j | 第 $j$ 个叶子的输出值（权重） | 叶子 D 输出 +2 |
...
GBDT 分裂时，靠"负梯度拟合残差"——这是启发式的。XGBoost 换了思路：

> 我的目标是最小化 Obj（损失 + 正则），那么**第 $t$ 棵树的最优叶子值**和**最优分裂点**，能不能从 Obj 直接算出来？

答案是肯定的——但前提是 Obj 的形式足够好（可对叶子值解析求导）。

问题在于 $L(y_i, \hat{y}_i^{(t-1)} + f_t(x_i))$ 的形式可以是任意损失，对 $f_t$ 求导复杂。

**解法：用二阶泰勒展开"打平"损失函数**，变成关于 $f_t$ 的二次多项式，就能解析求解。
...
**XGBoost 目标函数 = 损失 + 正则；对损失做二阶泰勒展开后，叶子最优值 $w_j^* = -G_j/(H_j+\lambda)$ 和分裂 Gain 都能直接从公式算出——停止分裂的条件（Gain ≤ 0）是数学内置的，不靠经验手调。**
