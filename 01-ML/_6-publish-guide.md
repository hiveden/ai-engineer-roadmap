...
```json
{
  "platforms": {
    ...
    "bilibili":        { "title": "...", "description": "...（含章节时间戳，无 #）", "tags": ["a","b",...] }
  }
}
```
...
| 平台 | tags 字段 | description 末尾 # | 章节时间戳 |
|---|---|---|---|
| douyin | ❌ 无 | ✅ `#xxx` 内联，**≤ 5 个**（硬约束） | ❌ 无 |
| xiaohongshu | ❌ 无 | ✅ `#xxx` 内联，6-9 个 | ❌ 无 |
| wechat_channels | ❌ 无 | ✅ `#xxx` 内联，2-4 个 | ❌ 无 |
| bilibili | ✅ 数组 8-10 个 | ❌ 不写 # | ✅ 必写（`MM:SS 标题` 行） |
